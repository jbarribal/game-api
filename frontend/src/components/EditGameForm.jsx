...
const EditGameForm = ({editGameData, onFormSubmit}) => {
  ...
  useEffect(() => {
      const releaseDate = new Date(editGameData.releaseDate);
      setFormData({
        title: editGameData.title,
        platforms: editGameData.platforms,
        releaseDate:releaseDate.toISOString().split('T')[0],
        libraryStatus: editGameData.libraryStatus,
        Rating: parseFloat(editGameData.Rating),
  ...
  const handleSubmit = (e) => {
    ...
    try {
      ...
      axios.put(`http://localhost:3000/api/v1/games/${editGameData.id}`, updatedFormData);
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-3 mx-auto mt-8 bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="mb-4">
        ...
        <select
          ...
          name="libraryStatus"
          onChange={handleChange}
          defaultValue={formData.libraryStatus}
          className="border rounded-lg px-4 py-2 w-full"
          required
        >
      ...
      <div className="text-center">
        <button type="submit" className="bg-green-600 text-white font-bold px-4 py-2 rounded-lg">
            Submit
        </button>
      </div>
    </form>
